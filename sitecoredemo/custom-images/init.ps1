...
Param (
    ...
    [string]
    $SitecoreAdminPassword = "Nvfkyy4(JBR4byKmxGDJ",
    
    # We do not need to use [SecureString] here since the value will be stored unencrypted in .env,
    # and used only for transient local example environment.
    [string]
    $SqlSaPassword = "e[|]3OQK&!~E!A7ipZH."
)
...
try {
    ...
    & $mkcert -install
    & $mkcert -key-file "cm.$($HostName).key" -cert-file "cm.$($HostName).crt" "cm.$($HostName)"
	& $mkcert -key-file "cd.$($HostName).key" -cert-file "cd.$($HostName).crt" "cd.$($HostName)"
	& $mkcert -key-file "hrz.$($HostName).key" -cert-file "hrz.$($HostName).crt" "hrz.$($HostName)"
	& $mkcert -key-file "id.$($HostName).key" -cert-file "id.$($HostName).crt" "id.$($HostName)"
}
catch {
